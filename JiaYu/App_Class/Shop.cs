...
public static class Shop
{
    ...
    public static string GetProductImage(string productNo)
    {
        ...
        {
            var pic = db.Photo.Where(m => m.pro_id == productNo).FirstOrDefault();
            string str_image = string.Format("~/Images/product/{0}", pic.filename);
            if (File.Exists(HttpContext.Current.Server.MapPath(str_image)))
                str_image = string.Format("~/Images/product/{0}", pic.filename);
            else
    ...
    public static List<Product> GetTopProducts()
    {
        using (JiaYuEntities db = new JiaYuEntities())
        {
            var lists = db.Product
              .Where(m => m.Is_top == true)
              .Where(m => m.is_sales == true)
              .OrderBy(m => m.pro_id)
              .ToList();
            return lists;
        }
    ...
    /// <returns></returns>
    public static List<Category> GetCategorys(string id)
    {
        using (JiaYuEntities db = new JiaYuEntities())
        {
            var lists = db.Category
                .Where(m => m.super_id == id)
                .OrderBy(m => m.cate_id)
                .ToList();
            return lists;
        }
    ...
    public static int GetParentCategoryList(string catrgoryNo, ref List<string> list_no, ref List<string> list_name)
    {
        using (JiaYuEntities db = new JiaYuEntities())
        {
            int int_id = 0;
            int int_count = 0;
            var model = db.Category.Where(m => m.cate_id == catrgoryNo).FirstOrDefault();
            if (model != null)
            {
                int_id = model.rowid;
                while (int_id > 0)
                {
                    var item = db.Category.Where(m => m.rowid == int_id).FirstOrDefault();
    ...
    */
    ...
    public static string GetCategoryName(string productNo)
    {
        ...
        using (JiaYuEntities db = new JiaYuEntities())
        {
            var prod = db.Product.Where(m => m.pro_id == productNo).FirstOrDefault();
            if (prod != null)
            {
                string string_cate_id = prod.categoryid.GetValueOrDefault();

                
                while (int_cate_id > 0)
                {
                    var cate = db.Category.Where(m => m.rowid == int_cate_id).FirstOrDefault();
                    if (cate == null) break;
    ...
    */
    ...
    public static string GetCategoryName(string categoryID)
    {
        using (JiaYuEntities db = new JiaYuEntities())
        {
            ...
            while (categoryID > 0)
            {
                var cate = db.Category.Where(m => m.rowid == categoryID).FirstOrDefault();
                if (cate == null) break;
    ...
    */
    ...
    /// <returns></returns>
    public static string GetCategoryName(string cat_no, ref string cat_id)
    {
        cat_id = "";
        string str_name = "";
        using (JiaYuEntities db = new JiaYuEntities())
        {
            var model = db.Category.Where(m => m.cate_id == cat_no).FirstOrDefault();
            if (model != null)
            {
                str_name = model.ca_name;
                cat_id = model.cate_id;
            }
            return str_name;
        }
    }
    ...
    public static void GetCategoryByProductNo(string productNo, ref string categoryNo, ref string categoryName)
    {
        ...
        using (JiaYuEntities db = new JiaYuEntities())
        {
            var model = db.Product.Where(m => m.pro_id == productNo).FirstOrDefault();
            
                var categotymodel = db.Category.Where(m => m.cate_id == model.categoryid).FirstOrDefault();
                if (categotymodel != null)
                {
                    categoryNo = categotymodel.cate_id;
                    categoryName = categotymodel.ca_name;
                }
    ...
    public static string GetProductName(string productNo)
    {
        ...
        using (JiaYuEntities db = new JiaYuEntities())
        {
            var models = db.Product.Where(m => m.pro_id == productNo).FirstOrDefault();
    ...
    public static string GetUserName(string UserEmail)
    {
        ...
        using (JiaYuEntities db = new JiaYuEntities())
        {
            var models = db.Member.Where(m => m.email == UserEmail).FirstOrDefault();
            if (models != null) str_name = models.m_name;
        }
        return str_name;
    }
    /// <summary>
    ...
    public static int GetProductPrice(string productNo)
    {
        ...
        using (JiaYuEntities db = new JiaYuEntities())
        {
            ...
            if (models != null)
            {
                if (models.price_sale > 0)
                    int_price = models.price_sale;
   ...
   */
    ...
    public static List<SelectListItem> GetPropertyList(string productNo)
    {
        using (JiaYuEntities db = new JiaYuEntities())
        {
            ...
            var models = db.ColorRelation.Where(m => m.pro_id == productNo).ToList();
            if (models != null)
            {
                foreach (var item in models)
                {
                    //str_name = item.color_no;
                    var datas = db.ColorList.Where(m => m.color_no == item.color_no).FirstOrDefault();
                    if (datas != null) str_name = datas.color_name;
                    SelectListItem data = new SelectListItem();
                    data.Text = str_name;
                    data.Value = item.color_no.ToString();
                    plist.Add(data);
                }
            }
            return plist;
    ...
    /// <param name="keyName">Key 值</param>
    /// <returns></returns>
    public static string GetColorName(string color_no)
    {
        int cr_no=0;
        int.TryParse(color_no,out cr_no);
        using (JiaYuEntities db = new JiaYuEntities())
        {
            string str_name = "";
            List<SelectListItem> plist = new List<SelectListItem>();
            var data = db.ColorList.Where(m => m.color_no == cr_no).FirstOrDefault();
            if(data != null) { 
                str_name = data.color_name;
            }
