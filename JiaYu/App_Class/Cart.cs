...
public static class Cart
{
            ...
            using (JiaYuEntities db = new JiaYuEntities())
            {
                ...
                if (datas != null)
                {
                    foreach (var item in datas)
                    {
                        ...
                        AddCart(item.pro_id, item.spec, int_qty);
    ...
    public static void AddCart(string productNo, string prod_Spec, int buyQty)
    {
        using (JiaYuEntities db = new JiaYuEntities())
        {
            ...
            var datas = db.Carts
                ...
                .Where(m => m.pro_id == productNo)
                .Where(m => m.spec == prod_Spec)
                .FirstOrDefault();
            ...
            if (datas == null)
            {
                ...
                models.lot_no = LotNo;
                models.user_id = UserAccount.user_id;
                models.create_time = LotCreateTime;
                models.pro_id = productNo;
                models.pname = Shop.GetProductName(productNo);
                models.spec = prod_Spec;
                models.qty = buyQty;
        ...
        using (JiaYuEntities db = new JiaYuEntities())
        {
            ...
            if (datas != null)
            {
                ...
                foreach (var item in datas)
                {
                    ...
                    detail.product_no = item.pro_id;
                    detail.product_name = item.pname;
                    //detail.vendor_no = Shop.GetVendorNoByProduct(item.product_no);
                    //detail.category_name = Shop.GetCategoryName(item.pro_id);
                    detail.product_spec = item.spec;
                    detail.qty = item.qty;
    ...
    private static string CreateNewOrderNo(cvmOrders model)
    {
        ...
        using (JiaYuEntities db = new JiaYuEntities())
        {
            ...
            orders.user_no = UserAccount.user_id;
